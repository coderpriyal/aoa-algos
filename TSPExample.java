...
{  
    ...
    static int findHamiltonianCycle(int[][] distance, boolean[] visitCity, int currPos, int cities, int count, int cost, int hamiltonianCycle, String currString)   
    {  
    
        if (count == cities && distance[currPos][0] > 0)   
        {  
            hamiltonianCycle = Math.min(hamiltonianCycle, cost + distance[currPos][0]);
            if(cost + distance[currPos][0] == hamiltonianCycle) answerString = currString;  
        ...
        {  
            ...
            {  
                ...
                // Mark as visited  
                visitCity[i] = true;  
                hamiltonianCycle = findHamiltonianCycle(distance, visitCity, i, cities, count + 1, cost + distance[currPos][i], hamiltonianCycle, currString + " " + (i + 1));  
    ...
    {  
        ...
        System.out.println("the minimum distance is :"+ hamiltonianCycle);  
        System.out.println("the minimum cost path is:" + "1" + answerString);  
    }
}
